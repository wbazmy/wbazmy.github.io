...
读的数据只能赋给Object。写数据时，实际上编译器不知道setFirst具体需要什么类型（真实类型），所以调用这个方法时不能接受类型为student或Object的参数，只能传递graduate类型的对象或者其子类型对象，因为最小是graduate，所以可以将graduate或子类赋给形参。

```java
public static void minmaxBonus(Manager[] a, Pair<? super Manager> result)
Pair<? super Manager> p=pe; //Pair<Employee>
Object obj=p.getFirst(); //只能赋给Object，因为真实类型可能是Manager的所有超类。
p.setFirst(new graduate()); //OK
```
