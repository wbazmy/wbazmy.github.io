...
## 8. 通配符类型
通配符类型允许类型参数变化。如：Pair&lt;? extends student&gt;。
...
但实际上编译器不知道setFirst具体需要什么类型，所以调用这个方法时不能接受类型为student或Object的参数，只能传递graduate类型的对象，或者某个子类型对象。
...
为了防止类型错误，带有超类型限定的通配符可以向泛型对象写入（防止读取不存在的变量），带有子类型限定的通配符可以从泛型对象读取（防止将子类域设置为父类对象）。  
### 无限定通配符
Pair&lt;?&gt;。getFirst返回值只能赋给一个Object。setFirst不能被调用，Object也不行。Pair与Pair&lt;?&gt;本质不同在于：可以用任意Object对象调用原始Pair类的setObject方法。  
不需要实际类型时可以使用无限定通配符。
### 通配符捕获
通配符不是类型变量，不能在编写代码时用？作为一种类型。可以用Pair&lt;T&gt; p捕获Pair&lt;?&gt; p来实现某些功能。
## 9. 反射和泛型
如果对象时泛型类的实例，关于泛型类型参数得不到太多信息，因为它们会被擦除。  
