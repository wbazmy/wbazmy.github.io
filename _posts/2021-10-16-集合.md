...
### Set总结
Set接口：存储无序的、不可重复的数据。Set接口没有额外新定义的方法，都用的是Collection的方法。添加到Set里的数据要重写equals和hashCode方法，且hashCode方法与equals方法尽量保持一致：相等的对象有一样的散列码。可用于数据的去重。  
...
#### HashMap的底层原理
key：用Set存，无序，不可重复。key所在的类要重写equals和hashCode方法（HashMap类）。  
value：无序，可重复。使用Collection存储。value所在的类要重写equals方法。  
key-value构成了一个Entry对象。entry无序不可重复，使用Set存。  
底层实现原理：jdk7：new HashMap()：实例化以后，底层创建了一个长度为16（初始长度小于等于16时）的一维数组Entry[] table。  
map.put(k1,v1):首先调用k1所在类的hashCode()和异或操作计算key1的 哈希值，哈希值通过与（位运算符效率高）操作计算后得到在Entry数组中的存储位置。如果此位置为空添加成功。不为空则比较key1和已存在的一个或多个数据（链表存储）的哈希值，如果哈希值都不同，添加成功。若找到与一个数据（k1,v1）哈希值相同，调用k1.equals(k2)，返回false添加成功，返回true则用v1替换v2。  
添加时与HashSet一样7上8下（jdk7：新元素作为table[loc]元素next指向原该位置元素）。
在不断的添加过程中，当超出临界值（当前长度*负载因子 （默认0.75））且要存放的位置非空时，会扩容，扩容为原来容量的2倍，并将原来的数据复制过来（要再哈希）。  
jdk8实现的不同：new HashMap：底层没有创建数组，节省内存。8中的底层数组为Node[]， 而不是Entry[]。  
首次调用put后会创建长度为16的数组。  
jdk7底层结构只有数组+链表，jdk8中底层结构：数组+链表+红黑树。当数组的某一个索引位置上的元素以链表形式存在的数据个数>8且当前数组长度>64时，此时此索引位置上的所有数据改为使用红黑树存储（查找快）。  
